...
This equation shows that the model anticipates an increase of `r round(hares_lm$coefficients[2], 2)` gram increase in hare weight for every millimeter increase in hind foot length. The model has an $R^2$ value of `r round(hares_lm_glance[1, "r.squared"], 3)`. This means that `r round(hares_lm_glance[1, "r.squared"], 3)*100` percent of the variance in the dependent variable (weight) is predictable from the independent variable (hind foot length). This provides further indication of earlier concerns that a simple linear regression does not explain the relationship between these variables very closely. Looking at the shape of the data in Figure 3 above, it is reasonable to think that a model with a slightly logarithmic curve could more accurately capture the relationship between Snowsheo Hare weight and foot length. 
